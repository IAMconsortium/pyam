...
class IamDataFrame(object):
    ...
    def timeseries(self, iamc_index=False):
        ...
        index = [i for i in s.index.names if i != self.time_col]
        return pd.concat(
            [_data.unstack(level=self.time_col) for _, _data in s.groupby(level=index)]
        )[self.time].rename_axis(None, axis=1)
